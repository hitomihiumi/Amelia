        ...
        try {
            if (client.holder.cmds.slashCommands.has(`${CategoryName}`)) {
                if (interaction.options.getSubcommand()) {
                    command = (client.holder.cmds.slashCommands.get(`${CategoryName}`) as unknown as Manifest ).commands[`${interaction.options.getSubcommand()}`] as unknown as SlashCommand;
                } else {
                    command = client.holder.cmds.slashCommands.get(`${CategoryName}`) as unknown as SlashCommand;
                }
            }
        } catch {
            if (client.holder.cmds.slashCommands.has(`${CategoryName}`)) {
