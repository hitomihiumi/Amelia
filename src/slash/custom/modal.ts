            ...
            if (i instanceof StringSelectMenuInteraction) {
                ...
                switch (i.customId) {
                    ...
                    case 'NI_modal:select_field':
                        if (i.values[0] === 'NI_modal:select_field:main') {
                            ...
                            msg.edit({ embeds: [embed], components: [fieldList(client, lang, _schema), ...editfieldrow], files: [fieldEmbed.attachment] });
                        }
                        break;
                }
                ...
                if (i.customId === 'NI_modal:page:prev') {
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:jump"
                    });
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:search"
                    });
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:title"
                    });
                    ...
                    // @ts-ignore
                    submit.update({embeds: [embed], components: [...editrow, fieldList(client, lang, _schema)]});
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:label"
                    });
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:placeholder"
                    });

                    _schema.fields[field].placeholder = submit.fields.getTextInputValue("NI_modal:text:placeholder");
                    ...
                    const submit = await i.awaitModalSubmit({
                        time: 5 * 60 * 1000,
                        filter: (si: any) => si.user.id === interaction.user.id && si.customId === "NI_modal:modal:sizes"
                    });
...
async function modalFieldEmbed(embed: EmbedBuilder, client: Client, lang: string, _schema: ModalCustom, field: number) {
    ...
    const attachment = new AttachmentBuilder(await new canvasUtil.ModalField(_schema.fields[field]).render(), {
        name: 'field.png'
    })
