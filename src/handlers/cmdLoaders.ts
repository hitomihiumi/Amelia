...
function groupCommandLoad(manifest: Manifest, dir: string) {
  ...
  readdirSync(`${process.cwd()}/dist/slash/${dir}/`).forEach((file) => {
    if (
      file.endsWith(".js") &&
      !lstatSync(`${process.cwd()}/dist/slash/${dir}/${file}`).isDirectory() &&
      file !== "manifest.js"
    ) {
...
function commandLoad(pull: SlashCommand, dir: string) {
  let slashCommand = new SlashCommandBuilder().setName(pull.name).setDescription(pull.description);
  if (pull.locale) {
    ...
    load(commands: any[]) {
      readdirSync(path.resolve(__dirname, "./../slash")).forEach((dir) => {
        if (lstatSync(path.resolve(__dirname, `./../slash/${dir}`)).isDirectory()) {
          const man = require(path.resolve(__dirname, `./../slash/${dir}/manifest.js`)) as {
            manifest: Manifest;
          };
