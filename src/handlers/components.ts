    ...
    try {
        readdirSync(path.resolve(__dirname, "./../components/")).forEach((dir) => {
            ...
            for (const file of components) {
                const pull = require(path.resolve(__dirname, `./../components/${dir}/${file}`)) as Button | Modal | SelectMenu | Autocomplete;
                if (pull.customId) {
                    switch (dir) {
                        case 'buttons':
        ...
        console.log(`Loaded `.green + `${client.holder.components.buttons.size}`.yellow + ` Buttons`.cyan);
        console.log(`Loaded `.green + `${client.holder.components.modals.size}`.yellow + ` Modals`.cyan);
        console.log(`Loaded `.green + `${client.holder.components.selectMenus.size}`.yellow + ` SelectMenus`.cyan);
        console.log(`Loaded `.green + `${client.holder.components.autocompletes.size}`.yellow + ` Autocompletes`.cyan);
    } catch (err: any) {
