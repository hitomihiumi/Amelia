 ...
 */
type PathValue<T extends GuildPath> =
  T extends 'settings.prefix' ? string :
  T extends 'settings.language' ? string :
  T extends 'utils.join_to_create.enabled' ? boolean :
  T extends 'utils.join_to_create.channel' ? string | null :
  T extends 'utils.join_to_create.category' ? string | null :
  T extends 'utils.join_to_create.default_name' ? string :
  T extends 'utils.counter.enabled' ? boolean :
  T extends 'utils.counter.channels' ? any :
  T extends 'utils.levels.enabled' ? boolean :
  T extends 'utils.levels.ignore_channels' ? string[] :
  T extends 'utils.levels.ignore_roles' ? string[] :
  T extends 'utils.components.modals' ? any[] :
  T extends 'moderation.moderation_roles' ? string[] :
  T extends 'permissions.commands' ? any :
  any;
